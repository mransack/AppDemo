jQuery.fn.table2CSV = function(options) {
    ...
    if (numCols > 0) {
        ...
        jQuery(el).filter(':visible').find(options.headerSelector).each(function () {
            ...
            if (content.indexOf("View Details") != "-1") {
                content = "http://" + document.domain + "/" + jQuery(this).find("a").attr("href");
            }
            if (jQuery(this).css('display') != 'none') tmpRow[tmpRow.length] = formatData(content);
        });
    }
